...
struct WayboundMapView: UIViewRepresentable {
    ...
    final class Coordinator: NSObject, MKMapViewDelegate, UIGestureRecognizerDelegate {
        ...
        /// New implementation uses grid-snapped corridor graph + barycenter sweep.
        private func ensureCorridorLaneLayouts() {
            ...
            // Step 1-2: build corridor graph from original shapes, builder densifies to 18 m internally.
            // Global node clustering merges 2-3 m divergent traces; publicKey grouping merges both directions of same route.
            let inputs = parent.journeys.enumerated().map { idx, journey in
            ...
            // Step 6: cache per feed version – live positions overlay without recompute.
            let graph = CorridorGraphCache.shared.graph(for: inputs)

            // Step 3-4: stable lane order via barycenter sweep already computed in cache,
            // then render with perpendicular offsets based on lane rank.
            var newLayouts: [Int: [CorridorLaneLayout]] = [:]
            ...
            for journey in parent.journeys {
                ...
                for polyline in journey.flagshipPolylines where polyline.count >= 2 {
                    ...
                    // Step 5: direction-aware – graph uses directed edges, so inbound/outbound stay distinct.
                    // Trunk owner is dominant public route per edge (first in ordered list).
                    var trunkOwner = Array(repeating: false, count: densified.count)
                    let myPublicKey = graph.routeInfo[journey.id]?.publicKey
                    for i in 0..<densified.count {
                        var ek: CorridorDirectedEdgeKey? = edgeKeys[i]
                        if ek == nil && i > 0 { ek = edgeKeys[i-1] }
                        if let key = ek, let edge = graph.edges[key] {
                            let distinctKeys = Set(edge.routeIDs.compactMap { graph.routeInfo[$0]?.publicKey ?? "id:\($0)" })
                            let isShared = distinctKeys.count >= 2 || edge.routeIDs.count >= 2
                            if isShared, let firstRep = edge.orderedRouteIDs.first {
                                if let firstInfo = graph.routeInfo[firstRep], let myPK = myPublicKey {
                                    trunkOwner[i] = firstInfo.publicKey == myPK
                                } else {
                                    trunkOwner[i] = firstRep == journey.id
                                }
                            }
                        }
                    }
                ...
                newLayouts[journey.id] = layouts
            }

            laneLayoutsByJourneyID = newLayouts
        }
